...
| # | 누를 곳 | 정상이면 보이는 것 |
| --- | --- | --- |
| 1 | **변경** → 합성 통보 목록 맨 위 '대표 데모'의 `X2 · 대표 데모 …` → **합성 통보 불러오기** | 카드 2장(같은 시기 외부 공지 N-X2, X2 통보). X2 카드에 "통보에서 읽어낸 변경 (확인 필요): T042 … 2026-03-14" |
| 2 | X2 카드의 **해석 확인 후 영향 분석** | **대응안** 화면으로 자동 이동. "통보 내용만 반영: 영향 없음 · 완료 2027-12-21", 아래에 "대응 불필요: 현재 일정으로 목표 충족" |
| 3 | 같은 화면의 **에이전트로 숨은 위험 조사** | 몇 초 뒤 한 줄 결론 "통보에 없던 P-C도 같은 수출 허가 대상일 수 있음 · 최악 +52일 · 제출 기한 2027-02-08"과 비교 카드 **통보 내용만 반영**(영향 없음) 대 **에이전트 조사 후**(P-C +52일 위험, 최악 완료 2028-02-11) |
| 4 | (선택) 접힌 **근거 문장 · 실제 기사**, **협력사 메일 초안**, **판단 기록** 펼치기 | 원문 강조와 근거 기사(CNBC 2025-10-15), 제출 기한이 들어간 메일 초안, 확인 4단계와 LLM 호출 수·비용 |
| 5 | 확인 요청 칸에 회신 내용 입력(선택) → **P-C도 해당함 · 일정 다시 계산** | "조사 결과 반영: 완료 2028-02-11 (+52일)". 대응안 목록 맨 위에 **추천** "제어 통합·교정 집중 투입 + 시운전·통합시험 병행 준비" **2028-01-25 · 회복 17일 · 32,000,000원**이 선택된 상태 |
| 6 | 오른쪽 **확인 요청 만들기** → 조건마다 회신 근거 입력 → **확인 기록** → **이 대응안 승인** | 실행 화면으로 이동 |
| 7 | **새 일정 버전 확정** → **Excel 다운로드** | 확정 버전 카드, `replan-….xlsx`의 '변경 일정' 시트 마지막 완료일 2028-01-25 |

5번에서 **P-C 해당 없음**을 고르면 통보 내용만 반영한 결과(2027-12-21)를 유지하고, **현재 일정 유지로 진행**으로 승인할 수 있습니다.
