...
협력사 통보 X2-C(2026-03-05, Equipment Vendor B): "T054 완료일이 2027-06-08에서 2027-06-18로 변경됩니다. 모듈 라인 현장 투입 인력의 교대 일정 때문입니다." 규칙이 T054 완료 2027-06-18을 추출하고 계산 결과는 2027-12-28(+7일)입니다. 같은 기간에 N-X2가 있어 연결 후보로 조회되지만, 사유(인력 일정)가 다르고 Vendor B 품목은 원산지 독일(EU 역내, 통관 대상 아님)이라 **연결하지 않고 M1**. 결과는 규칙만과 같고, 확인할 것은 "B·C 같은 추가 품목을 만들어내지 않음"입니다.

통보 문구 주의: "T054 … 시운전 착수를 … 늦춥니다"처럼 쓰면 지금 규칙 해석기가 T054를 무시하고 T038·T039·T052에 착수 제한을 겁니다(시험 작업 키워드 목록에 `commissioning`이 없어, 명시된 작업 ID가 키워드 일치 작업으로 대체됨. `services/api/app/events.py`의 시험 작업 분기). 사람 확인 단계에서 잡을 수 있지만 규칙 결함이므로 구현 단계에서 함께 고칩니다.
...
```json
{
  ...
  "supplier_messages": [
    ...
    {"event_id": "X2-C", "published_at": "2026-03-05T09:00:00+01:00", "channel": "supplier_message",
     ...
     "content": "[가상 메시지] T054 완료일이 2027-06-08에서 2027-06-18로 변경됩니다. 모듈 라인 현장 투입 인력의 교대 일정 때문입니다."}
  ]
}
```
...
0. 규칙 해석기: 명시된 작업 ID가 있는 시험·시운전 통보에서 ID를 우선(7.4절 주의).
